{{--
    Deliberately standalone: no app layout, no components, no database.

    An error page renders precisely when something is wrong, and the app layout
    reads settings for branding, resolves the signed-in user and pulls in the
    nav. Any one of those can be the thing that just broke, and an error page
    that throws while rendering an error gives the framework's raw page instead,
    which on a misconfigured install is how source ends up on screen.

    So this is one file, inline CSS, nothing to resolve.
--}}
<!doctype html>
<html lang="en">
<head>
...
<body>
    <main class="card">
        ...
        <div class="actions">
            ...
            @else
                <a class="btn primary" href="{{ url('/') }}">Back To My Servers</a>
            @endif
