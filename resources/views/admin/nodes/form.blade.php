<x-layouts.app :title="$title">
    <x-page-header :title="$title" icon="server"
                   subtitle="Five short steps. Nothing here needs scrolling." />
    ...
    <form method="POST" action="{{ $node->exists ? route('admin.nodes.update', $node) : route('admin.nodes.store') }}"
        ...
        <div x-show="step === 4" x-cloak>
            <x-card title="Capacity"
                ...
                <div class="grid gap-4 sm:grid-cols-2 max-w-2xl">
                    ...
                    <x-field label="Memory Over-allocation (%)" required hint="0 never promises more than exists.">
        ...
        <div x-show="step === 5" x-cloak>
            <x-card title="Placement" subtitle="Whether servers land here on their own.">
                <div class="space-y-4 max-w-2xl">
                    ...
                    <x-field label="Data Directory" required hint="Where server files live on the machine.">
        ...
        <div x-show="editing && step !== 5" x-cloak class="mt-4 flex justify-end">
            <x-button type="submit" variant="secondary" size="sm" icon="check">Save Node</x-button>
        </div>
    </form>
</x-layouts.app>
