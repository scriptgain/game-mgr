...
# =============================================================================
APP_ID=1422450
STEAM_LOGIN="${STEAM_LOGIN:-${STEAM_USER:-}}"
: "${STEAM_LOGIN:?a Steam account is required: bind this server to one in the panel}"
STEAM_PASSWORD="${STEAM_PASSWORD:-${STEAM_PASS:-}}"
: "${STEAM_PASSWORD:?a Steam password is required: bind this server to a Steam account in the panel}"
# Minted by the panel at dispatch and good for about thirty seconds.
STEAM_GUARD_CODE="${STEAM_GUARD_CODE:-${STEAM_2FA_CODE:-}}"
...
#
# Upstream retries three times with a five second gap. Against a Steam Guard
# protected account that is three rejected codes in fifteen seconds, and Steam
# answers a run of those with a rate limit that looks exactly like a bad
# password and lasts a great deal longer than the install would have. A failed
# login is a reason to stop and say so, not to try again harder.
#
# set_steam_guard_code is written into a runscript rather than passed as an
# argument, because argv is world readable in /proc for as long as the download
# runs, and this download is measured in tens of gigabytes.
RUNSCRIPT="$(gosu steam mktemp)"
{
    echo "@ShutdownOnFailedCommand 1"
    echo "@NoPromptForPassword 1"
    echo "@sSteamCmdForcePlatformType windows"
    echo "force_install_dir ${INSTALL_DIR}"
    # Before login: it primes the value the login consumes, and afterwards there
    # is nothing left to answer.
    [ -n "${STEAM_GUARD_CODE}" ] && echo "set_steam_guard_code ${STEAM_GUARD_CODE}"
    echo "login ${STEAM_LOGIN} ${STEAM_PASSWORD}"
    echo "app_update ${APP_ID}"
    echo "quit"
} > "$RUNSCRIPT"
chmod 600 "$RUNSCRIPT"
chown steam:steam "$RUNSCRIPT"

echo "[phase 2] SteamCMD..."
gosu steam /home/steam/steamcmd/steamcmd.sh +runscript "$RUNSCRIPT"
STEAM_RC=$?
rm -f "$RUNSCRIPT"

if [ $STEAM_RC -ne 0 ]; then
    echo "[phase 2] SteamCMD exited ${STEAM_RC}."
    echo "[phase 2] If Steam asked for a Guard code, bind this server to a Steam account"
    echo "[phase 2] with a shared secret stored, or authorise this node once by hand."
fi
...
DOTNET_CACHE="/home/steam/dotnet-cache"
