...
<head>
    ...
    <title><?php echo e($page->headline ?: $server->name); ?></title>
    <style>
        :root {
...
<div class="card">
    <div class="top">
        <div style="min-width:0">
            <p class="name"><?php echo e($page->headline ?: $server->name); ?></p>
            <?php if($server->template?->game?->name): ?>
                <p class="game"><?php echo e($server->template->game->name); ?></p>
            <?php endif; ?>
        </div>
        <span class="pill <?php echo e($server->power_state === 'running' ? 'on' : ''); ?>">
            <span class="dot"></span><?php echo e($server->power_state === 'running' ? 'Online' : 'Offline'); ?>
    ...
    <div class="rows">
        <?php if($page->show_address): ?>
            <div class="row">
                ...
                <span class="v mono"><?php echo e($server->connectAddress() ?: $server->address()); ?></span>
            </div>
        <?php endif; ?>
        <?php if($page->show_players): ?>
            <div class="row">
                <span class="k">Players</span>
                <span class="v"><?php echo e((int) $server->cached_players); ?><?php if($server->cached_max_players): ?> / <?php echo e((int) $server->cached_max_players); ?><?php endif; ?></span>
            </div>
        <?php endif; ?>
        <?php if($page->show_version && $server->template?->name): ?>
            <div class="row">
                <span class="k">Running</span>
                <span class="v"><?php echo e($server->template->name); ?></span>
            </div>
        <?php endif; ?>
    </div>
</div>
...
<?php /**PATH /var/www/gamemgr/resources/views/status-embed.blade.php ENDPATH**/ ?>